          ...
          <tr className="text-slate-500 border-b bg-slate-50/40">
            ...
            {showHouse && (
              <>
                ...
                <th className="py-2.5 p-2">Net area (m²)</th>
                <th className="py-2.5 p-2">Proportional area (m²)</th>
                <th className="py-2.5 p-2">Common area (m²)</th>
                <th className="py-2.5 p-2">Total area (m²)</th>
                <th className="py-2.5 p-2">House area (m²)</th>
              </>
            )}
          </tr>
        </thead>
            ...
            <tr key={r.id || i} className="border-b last:border-0 hover:bg-slate-50 font-medium text-slate-700">
              ...
              {showHouse && (
                <>
                  ...
                  <td className="p-2 font-mono font-bold text-[#059669]">{r.area || '—'}</td>
                </>
              )}
            </tr>
          ))}
...
function handleDownload(e) {
  ...
  fetch(href, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
    .then(async (r) => {
      ...
      // Extract custom filename from header
...
function fakeHrefDownload(e, customId) {
  ...
  fetch(url, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
    .then(async (r) => {
      ...
      // FIXED: Read filename from server headers instead of hardcoding 'CBE_Lottery_Result.xlsx'
      const dispo = r.headers.get('Content-Disposition') || '';
      const m = /filename="([^"]+)"/.exec(dispo);
      const filename = m ? m[1] : 'lottery_run_result.xlsx';
