...
export default function LotteryPage() {
  ...
  const [rawApplicantsForCombo, setRawApplicantsForCombo] = useState([]);
  ...
  // Fetch individual applicants dynamically when combo is selected
  useEffect(() => {
    if (!siteId || !bedType || !totalArea) {
    ...
    (async () => {
      ...
      try {
        ...
        console.error("Failed to verify live applicant status rules", e);
      } finally {
  ...
  const isAlreadyDrawn = useMemo(() => {
    ...
    const currentKey = `${String(siteId).trim()}|${Number(bedType)}|${String(totalArea).trim()}`;
    return drawnKeys.has(currentKey);
          ...
          <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-5">
            ...
            {loadingApplicants ? (
              <div className="mt-2 text-sm text-slate-500 flex items-center gap-2">
                ...
                Validating eligible applicant records...
              </div>
            ) : isAlreadyDrawn ? (
              ...
              <div className="mt-2 text-sm text-slate-500">
                Choose parameters to preview the draw.
              </div>
            ) : preview.appCount === 0 ? (
              <div className="mt-2 text-sm font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
                ⚠️ There is no applicant for this house detail.
